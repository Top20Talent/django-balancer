...
class PinningSessionMiddleware(MiddlewareMixin):
    """
    ...
    def process_request(self, request):
        ...
        if pinned_until and datetime.strptime(pinned_until, self.DATETIME_FORMAT) > datetime.now():
            pinning.pin_thread()
    ...
    def process_response(self, request, response):
        ...
        if pinning.db_was_written():
            ...
            request.session[PINNING_KEY] = pinned_until.strftime(self.DATETIME_FORMAT)
            pinning.clear_db_write()
        pinning.unpin_thread()
        return response


class PinningCookieMiddleware(MiddlewareMixin):
    """
